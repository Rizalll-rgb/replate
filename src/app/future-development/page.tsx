...
function FutureDevelopmentContent() {
  ...
  const feature = searchParams.get('feature') || 'google-oauth';
  const from = searchParams.get('from') || 'login';

  const returnUrl = from === 'register' ? '/register' : '/login';
        ...
        <div className="bg-[#142C47]/90 backdrop-blur-xl border-1.5 border-[#2C5A8F] rounded-3xl p-6 sm:p-10 shadow-2xl space-y-6 text-left text-white">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed pt-1">
              Terima kasih atas ketertarikan Anda! Fitur <strong>Masuk / Daftar Cepat Menggunakan Akun Google</strong> saat ini sedang dipersiapkan dalam roadmap pengembangan tahap selanjutnya (Replate Enterprise Release v2.1).
            </p>
          </div>
          ...
          <div className="p-4 bg-[#0F1923] rounded-2xl border border-slate-800 space-y-3">
            ...
            <div className="space-y-2.5 text-xs text-slate-300">
              <div className="flex items-start gap-2.5">
                ...
                <span>
                  <strong>Standarisasi e-KYC Dinsos RI:</strong> Sinkronisasi data identitas lembaga sosial dan NIB usaha secara otomatis dengan verifikasi sertifikasi dua arah.
                </span>
              </div>
              <div className="flex items-start gap-2.5">
                <Layers className="w-4 h-4 text-sky-400 shrink-0 mt-0.5" />
                <span>
                  <strong>Pemisahan Multi-Peran Google Workspace:</strong> Validasi domain email instansi (restoran, hotel, yayasan) agar terpisah dengan akun personal konsumen.
                </span>
              </div>
              <div className="flex items-start gap-2.5">
                <Calendar className="w-4 h-4 text-[#D4A843] shrink-0 mt-0.5" />
                <span>
                  <strong>Estimasi Peluncuran:</strong> Siap diluncurkan pada rilis kuartal ketiga (Q3 2026) secara nasional.
                </span>
              </div>
            </div>
          </div>

          {/* Current Available Access Options */}
          <div className="p-4 bg-[#1B3A5C]/40 rounded-2xl border border-[#2C5A8F]/60 space-y-2">
            <span className="text-xs font-black text-emerald-300 uppercase tracking-wider block">
              Cara Masuk yang Siap Digunakan Saat Ini:
            </span>
            <div className="space-y-2 text-xs text-slate-300">
              <div className="flex items-start gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                <span>
                  <strong>Mode Demo 1-Klik (Juri Penilai):</strong> Anda dapat langsung masuk tanpa repot menggunakan 5 akun siap pakai untuk Food Provider, Yayasan, Konsumen, Relawan, atau SuperAdmin.
                </span>
              </div>
              <div className="flex items-start gap-2">
                <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                <span>
                  <strong>Pendaftaran Akun Manual:</strong> Gunakan form registrasi dengan email dan verifikasi OTP WhatsApp yang telah aktif 100%.
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center gap-3">
            <Link href={returnUrl} className="w-full sm:flex-1">
              <button
                type="button"
                className="w-full py-3 px-5 bg-[#D4A843] hover:bg-[#E5B954] text-slate-950 font-black text-xs rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
                <ArrowLeft className="w-4 h-4" />
                <span>Kembali ke Halaman {from === 'register' ? 'Registrasi' : 'Login'}</span>
              </button>
            </Link>
