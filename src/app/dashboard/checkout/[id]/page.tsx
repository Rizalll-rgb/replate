...
export default function CheckoutPage() {
  ...
  const handleAutoPayDemo = () => {
    ...
    setTimeout(() => {
      try {
        const newClaim = buildClaim(uploadProofModal.resiCode, 'READY_FOR_PICKUP', 'https://images.unsplash.com/photo-1554415707-9e4466b88738?w=500&auto=format&fit=crop&q=60');
        persistClaim(newClaim);
        setActionLoader({ isOpen: false, message: '' });
        setSuccessModal({ isOpen: true, claim: newClaim });
        setToastState({
  ...
  return (
    <>
      ...
      <div className="space-y-6 max-w-4xl mx-auto pb-36 lg:pb-12">
      ...
      <div className="lg:hidden fixed bottom-[60px] left-0 right-0 z-40 bg-white/95 backdrop-blur-lg border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] safe-area-pb animate-fade-in-up">
        <div className="px-4 py-2.5 flex items-center justify-between gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[9px] font-medium text-slate-400 uppercase tracking-wider">Total Tagihan ({quantity} Porsi)</p>
          ...
          <button
            onClick={handleCheckout}
            disabled={isCheckingOut || isItemOutOfStock}
            className={`shrink-0 font-black text-xs px-5 py-3 rounded-xl shadow-lg transition-all active:scale-[0.97] ${
              isItemOutOfStock
                ? 'bg-slate-200 text-slate-400 cursor-not-allowed border border-slate-300'
                : 'bg-gradient-to-r from-[#D4A843] to-[#B8902E] hover:from-[#B8902E] hover:to-[#8C6D22] text-slate-950 shadow-[#D4A843]/25 disabled:opacity-50 disabled:cursor-not-allowed'
            }`}
          >
            {isItemOutOfStock ? (
              'Porsi Habis'
        ...
        <div className="space-y-3.5 text-xs text-center font-sans">
          <div className="p-4 bg-gradient-to-br from-slate-50 to-blue-50/50 rounded-2xl border border-blue-200 space-y-3">
            <div className="flex items-center justify-center gap-2 text-[#1B3A5C]">
              <QrCodeIcon size={16} />
              <span className="font-black text-sm">QRIS Pembayaran Resmi — {item?.providerName}</span>
            </div>
            ...
            <div className="p-3 bg-white rounded-xl border border-slate-200 text-left space-y-1.5">
              <div className="flex justify-between"><span className="text-slate-500">Nama Toko:</span><strong className="text-slate-800">{providerQris.merchantName || item?.providerName}</strong></div>
              ...
              <div className="flex justify-between"><span className="text-slate-500">Nomor Resi:</span><strong className="text-slate-700 font-mono text-xs">{uploadProofModal.resiCode}</strong></div>
            </div>

            <p className="text-[10.5px] text-slate-500 leading-snug">
              Buka aplikasi e-wallet atau m-Banking Anda, pilih menu <strong>Scan QRIS</strong>, dan arahkan kamera ke kode barcode di atas.
            </p>
          </div>
