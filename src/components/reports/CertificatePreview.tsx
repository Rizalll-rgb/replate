    ...
    <div className="space-y-4 max-w-3xl mx-auto">
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-4 bg-white/80 p-4 rounded-xl border border-[#C5A059]/40 max-w-xl mx-auto text-center shadow-xs relative z-10">
          <div>
            ...
            <span className="text-xl font-black text-[#1B3A5C]">{data.totalSavedKg || 42.5} Kg</span>
          </div>
          <div className="border-x border-slate-200 px-2">
            ...
            <span className="text-xl font-black text-emerald-700">{data.totalCo2SavedKg || 106.3} Kg</span>
          </div>
          <div>
            <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider block">Penerima Manfaat</span>
            <span className="text-xl font-black text-[#C5A059]">{data.totalPeopleFed || 85} Jiwa</span>
          </div>
        </div>

        {/* Natural Official Signatures */}
        <div className="grid grid-cols-3 items-end pt-6 border-t border-[#C5A059]/30 text-xs relative z-10">
          ...
          <div className="flex flex-col items-center justify-center">
            <div className="w-16 h-16 rounded-full border-2 border-[#C5A059] bg-[#FCFBF7] flex flex-col items-center justify-center shadow-sm">
              <span className="text-lg">🎖️</span>
              <span className="text-[7px] font-black text-[#C5A059] tracking-tighter uppercase">REPLATE AUDITED</span>
            </div>
          </div>

          <div className="text-center space-y-1">
            <div className="font-serif font-bold text-[#1B3A5C] italic text-sm">Rizal & Daffa</div>
            <p className="font-bold text-[#1B3A5C] text-[11px]">Tim Pengembang Replate</p>
      ...
      <div className="flex justify-center gap-3 no-print">
        <Button variant="gold" size="md" className="font-extrabold flex items-center gap-2 shadow-md" onClick={handlePrint}>
          ...
          <span>🖨️ Cetak Sertifikat Penghargaan (1 Halaman Clean)</span>
        </Button>
      </div>
    </div>
  );
